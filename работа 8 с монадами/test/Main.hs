...
main = do
  ...
  run "monad left identity" testMonadLeftIdentity
  run "monad right identity" testMonadRightIdentity
  run "monad associativity" testMonadAssociativity
...
intTree :: Tree Int
intTree = Node (Node Empty 1 Empty) 2 (Node Empty 3 Empty)

f :: Int -> Tree Int
f x = Node Empty (x + 10) (Node Empty (x + 20) Empty)

g :: Int -> Tree Int
g x = Node (Node Empty (x * 2) Empty) (x * 3) Empty

testMonadLeftIdentity :: Bool
testMonadLeftIdentity = eqByInorder (return 5 >>= f) (f 5)

testMonadRightIdentity :: Bool
testMonadRightIdentity = eqByInorder (intTree >>= return) intTree

testMonadAssociativity :: Bool
testMonadAssociativity =
  eqByInorder ((intTree >>= f) >>= g) (intTree >>= (\x -> f x >>= g))

eqByInorder :: Eq a => Tree a -> Tree a -> Bool
eqByInorder t1 t2 = inorder t1 == inorder t2
